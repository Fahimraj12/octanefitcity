...
export default function EquipmentList() {
    ...
    const fetchEquipments = async () => {
        try {
            ...
            const res = await apiRequest.get("equipment/get-equipment");

            if (res.status?.toUpperCase() === "SUCCESS") {
                setEquipments(res.result);
            } else {
                setEquipments([]);
            }

        } catch (err) {
            alert.error("Failed to load equipments");
        } finally {
            rootCtx[0](false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            await apiRequest.delete(`equipment/delete-equipment/${id}`);
    ...
    const filteredEquipments = useMemo(() => {
        return equipments.filter(eq => {
            const matchesSearch =
                eq.title?.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const totalPages = Math.ceil(filteredEquipments.length / recordsPerPage);

    const paginatedEquipments = useMemo(() => {
        const start = (currentPage - 1) * recordsPerPage;
        return filteredEquipments.slice(start, start + recordsPerPage);
        ...
        <Container>
            ...
            <HeaderSection>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    <PageTitle>
                        Fitness <span>/ Equipment</span>
                    </PageTitle>
                    <Sub>Manage gym equipment inventory and statuses.</Sub>
                </div>
            ...
            <FilterCard>
                <FormGroup style={{ flex: 2 }}>
                    ...
                    <InputWrapper>
                        ...
                        <Input
                            type="text"
                            placeholder="Search equipment title or description..."
                            value={search}
                ...
                <Table>
                    <thead>
                        <tr>
                            ...
                            <th width="80" className="text-center">Image</th>
                            ...
                            paginatedEquipments.map((eq, index) => (
                                <tr key={eq.id}>
                                    ...
                                    <td className="text-center">
                                        <ImageWrap>
                                            <img
                                                src={`http://localhost:5000/uploads/levels/${eq.image}`}
                                                alt={eq.title}
                                                onError={(e) => {
                                                    e.target.onerror = null;
                                                    e.target.src = "https://via.placeholder.com/70x50?text=No+Img";
                                                }}
                                            />
                                        </ImageWrap>
                                    </td>
                                    <td>
                                        <TitleText>{eq.title}</TitleText>
                                    </td>
                                    <td>
                                        <DescText>{eq.description}</DescText>
                                    </td>
                                    <td className="text-center">
                                        <Badge
                                            $variant={
                                                eq.status === "active" ? "success" : "danger"
                                            }
                                        >
                                            {eq.status}
                                        </Badge>
                                    ...
                                    <td className="text-center">
                                        <ActionContainer>
                                            <MenuButton
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setActiveActionId(activeActionId === eq.id ? null : eq.id);
                                                }}
                                            >
                                                <i className="bi bi-three-dots-vertical" />
                                            </MenuButton>

                                            {activeActionId === eq.id && (
                                                <ActionDropdown>
                                                    <DropdownItem
                                                        onClick={() => {
