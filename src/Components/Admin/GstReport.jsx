...
export default function GstReport() {
    ...
    const generateReport = async () => {
        ...
        try {
            rootCtx[0](true);
            const url = `reports/gst-report?startDate=${startDate}&endDate=${endDate}`;
    ...
    const exportToExcel = () => {
        ...
        const headers = ["Date", "Invoice No", "Taxable (Gross)", "GST Percentage", "GST Amount", "Net Total"];
        const rows = data.map(row => [
            new Date(row.receipt_date).toLocaleDateString(),
            row.invoice_no,
            row.gross_amount,
            `${row.gst_percentage}% (${row.gst_type})`,
            row.gst_amount,
            row.net_amount
        ]);
        rows.push(["TOTAL LIABILITY", "", totals.gross_amount, "-", totals.gst_amount, totals.net_amount]);
        ...
        XLSX.writeFile(workbook, `GST_Report_${startDate}_to_${endDate}.xlsx`);
    };
        ...
        <Container>
            ...
            <HeaderSection>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    <PageTitle>
                        Reports <span>/ GST Report</span>
                    </PageTitle>
                    <Sub>Generate and export GST liability data by date range.</Sub>
                </div>
                {data.length > 0 && (
            ...
            {data.length > 0 ? (
                    ...
                    <Table>
                        <thead>
                            <tr>
                                ...
                                <th>Invoice No</th>
                                <th className="text-end">Taxable (Gross)</th>
                                <th className="text-center">GST Percentage</th>
                                <th className="text-end">GST Amount</th>
                                <th className="text-end pe-4">Net Total</th>
                            </tr>
                        </thead>
                        <tbody>
                            {data.map((row, i) => (
                                <tr key={i}>
                                    ...
                                    <td className="text-end">
                                        <AmountDanger>₹{row.gst_amount}</AmountDanger>
                                    </td>
                        ...
                        <TableFooter>
                            <tr>
                                <td colSpan="2" className="text-end ps-4 label">TOTAL LIABILITY:</td>
                                <td className="text-end value base">₹{totals?.gross_amount?.toFixed(2)}</td>
                ...
                <EmptyState>
                    ...
                    <p>Select a start and end date, then click Generate to view the GST report.</p>
                </EmptyState>
            )}
        </Container>
    );
...
const TableFooter = styled.tfoot`
  ...
  .tax {
    color: #dc2626; /* Danger color indicating tax liability */
  }
