...
export default function Level() {
    ...
    const fetchLevels = async () => {
        try {
            rootCtx[0](true);
            const res = await apiRequest.get("level/get-levels");
            ...
            alert.error("Failed to load levels");
        } finally {
            rootCtx[0](false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            rootCtx[0](true);
            await apiRequest.delete(`level/delete-level/${id}`);
    ...
    const filteredLevels = useMemo(() => {
        return levels.filter(level => {
            ...
            const matchesStatus =
                !filters.status || level.status === filters.status;
    ...
    // PAGINATION
    const totalPages = Math.ceil(filteredLevels.length / recordsPerPage);

    const paginatedLevels = useMemo(() => {
        const start = (currentPage - 1) * recordsPerPage;
        return filteredLevels.slice(start, start + recordsPerPage);
        ...
        <Container>
            ...
            <HeaderSection>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    <PageTitle>
                        Fitness <span>/ Levels</span>
                    </PageTitle>
                    <Sub>Manage fitness levels and categories.</Sub>
                </div>
                            ...
                            paginatedLevels.map((level, index) => (
                                <tr key={level.id}>
                                    ...
                                    <td className="text-center">
                                        <ImageWrap>
                                            <img
                                                src={`http://localhost:5000/uploads/levels/${level.image}`}
                                                alt={level.title}
                                                onError={(e) => {
                                    ...
                                    <td>
                                        <TitleText>{level.title}</TitleText>
                                    </td>
                                    <td>
                                        <DescText>{level.description}</DescText>
                                    </td>
                                    <td className="text-center">
                                        <Badge
                                            $variant={
                                                level.status === "active" ? "success" : "danger"
                                            }
                                        >
                                            {level.status}
                                        </Badge>
                                    ...
                                    <td className="text-center">
                                        <ActionContainer>
                                            <MenuButton
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setActiveActionId(activeActionId === level.id ? null : level.id);
                                                }}
                                            >
                                                <i className="bi bi-three-dots-vertical" />
                                            </MenuButton>

                                            {activeActionId === level.id && (
                                                <ActionDropdown>
                                                    <DropdownItem
                                                        onClick={() => {
                            ...
                            <tr>
                                <td colSpan="6">
                                    <EmptyState>
                                        ...
                                        <p>Try adjusting your search criteria or add a new level.</p>
                                    </EmptyState>
                                </td>
                            </tr>
                        )}
