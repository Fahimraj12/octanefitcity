...
import MembershipPackageModal from "../Modals/MembershippackageModal";
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            await apiRequest.delete(
                `membershippackage/delete-membershippackages/${id}`
            );
    ...
    const filteredPackages = useMemo(() => {
        return packages.filter(pkg => {
            const matchesSearch =
                ...
                pkg.duration?.toLowerCase().includes(search.toLowerCase());

            const matchesType =
                !filters.membership_type ||
                pkg.membership_type?.toLowerCase().includes(filters.membership_type.toLowerCase());
        ...
        <Container>
            ...
            <HeaderSection>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    <PageTitle>
                        Memberships <span>/ Packages</span>
                    </PageTitle>
                    <Sub>Manage gym membership plans and pricing</Sub>
                </div>
            ...
            <FilterCard>
                <FormGroup style={{ flex: 2 }}>
                    ...
                    <InputWrapper>
                        ...
                        <Input
                            type="text"
                            placeholder="Search name or duration..."
                            value={search}
                ...
                <FormGroup style={{ flex: 1 }}>
                    ...
                    <InputWrapper>
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                setFilters({ ...filters, membership_type: e.target.value })
                            }
                        />
                    </InputWrapper>
                </FormGroup>
                        ...
                        {paginatedPackages.length > 0 ? (
                            paginatedPackages.map((pkg, index) => (
                                <tr key={pkg.id}>
                                    ...
                                    <td>
                                        <TitleText>{pkg.name}</TitleText>
                                    </td>
                                    <td>
                                        <TypeBadge>{pkg.membership_type}</TypeBadge>
                                    </td>
                                    <td className="fw-semibold text-muted">{pkg.duration}</td>
                                    ...
                                    <td className="text-center">
                                        {pkg.gst_status === 'included' ? (
                                            <GstBadge $inc>Inc ({pkg.gst_percentage}%)</GstBadge>
                                        ) : (
                                            <GstBadge>Excluded</GstBadge>
                                        )}
                                    </td>
                                    ...
                                    <td className="text-center">
                                        <Badge
                                            $variant={
                                                pkg.status === "active" ? "success" : "danger"
                                            }
                                        >
                                            {pkg.status}
                                        </Badge>
                            ...
                            <tr>
                                <td colSpan="10">
                                    <EmptyState>
                                        ...
                                        <h6>No Membership Packages Found</h6>
...
const Table = styled.table`
  ...
  min-width: 1000px; /* Wider to accommodate 10 columns cleanly */
