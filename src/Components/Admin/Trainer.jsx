...
export default function Trainer() {
    ...
    const fetchTrainers = async () => {
        try {
            rootCtx[0](true);
            const res = await apiRequest.get("trainer/get-trainers");
            ...
            alert.error("Failed to load trainers");
        } finally {
            rootCtx[0](false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            rootCtx[0](true);
            await apiRequest.delete(`trainer/delete-trainer/${id}`);
    ...
    const filteredTrainers = useMemo(() => {
        return trainers.filter(trainer => {
            const matchesSearch =
                ...
                trainer.mobile?.includes(search);

            const matchesStatus =
                !filters.status || trainer.status === filters.status;
    ...
    const paginatedTrainers = useMemo(() => {
        ...
        return filteredTrainers.slice(start, start + recordsPerPage);
                            ...
                            {paginatedTrainers.length > 0 ? (
                                paginatedTrainers.map((trainer, index) => (
                                    <tr key={trainer.id}>
                                        ...
                                        <td>
                                            <GenderText>{trainer.gender}</GenderText>
                                        </td>
                                        <td>
                                            <Badge
                                                $variant={
                                        ...
                                        <td className="text-center">
                                            <ActionContainer>
                                                <MenuButton
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        setActiveActionId(activeActionId === trainer.id ? null : trainer.id);
                                                    }}
                                                >
                                                    <i className="bi bi-three-dots-vertical" />
                                                </MenuButton>

                                                {activeActionId === trainer.id && (
                                                    <ActionDropdown>
                                                        <DropdownItem
                                                            onClick={() => {
