...
export default function EquipmentModal({ show, handleClose, onSuccess, editData }) {
  const rootCtx = useContext(rootContext);
  const [preview, setPreview] = useState(null);

  const schema = Yup.object({
    title: Yup.string().required("Required"),
    image: editData ? Yup.mixed().nullable() : Yup.mixed().required("Image is required"),
      ...
      try {
        ...
        if (editData) {
          response = await apiRequest.put(
            `equipment/update-equipment/${editData.id}`,
            formData
          );
        } else {
          response = await apiRequest.post(`equipment/add-equipment`, formData);
        }

        if (response.status?.toUpperCase() === "SUCCESS") {
          alert.success(editData ? "Equipment Updated" : "Equipment Added", "Octane GYM");
          resetForm();
          setPreview(null);
          handleClose();
          onSuccess();
        } else {
          alert.error(response.result || "Failed to save equipment");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (editData) {
      ...
      if (editData.image) {
        setPreview(`http://localhost:5000/uploads/levels/${editData.image}`);
      } else {
        setPreview(null);
      }
    } else {
    ...
    <Overlay>
      <ModalWrapper>
        <ModalContent>
          <ModalHeader>
            <HeaderTitle>
              <TitleIcon className="bi bi-tools" />
              <TitleText>
            ...
            <ModalBody>
              <FormGrid>
                ...
                <FormGroup className="span-12">
                  ...
                  <Textarea
                    rows="4"
                    placeholder="Maintenance notes, specifications, or usage instructions..."
